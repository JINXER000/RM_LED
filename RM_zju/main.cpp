...
int frame_process()
{
...
#ifdef _DBG1FRAME_
	srcImg=imread("../dbg_imgs/1false_marksense.jpg", 1);
	if (srcImg.empty()) {
		printf("Image empty !\n");
		return -1;
	}
	//resize(srcImg, bgrImg, dist_size);
	srcImg.copyTo(bgrImg);
...
#else
	VideoCapture capture;	//定义关于视频处理类VideoCapture的实例
#ifdef TX2
	capture.open(1);
#else 
	...
	while (true)
	{
		...
		///detect and track
		resize(srcImg, bgrImg, dist_size);
		isSuccess = markSensor.ProcessFrameLEDXYZ(bgrImg, X, Y, Z, led_type,pix_x,pix_y);
		if (isSuccess)
		{
			cout << "detected target" << endl;
			X_bias = pix_x - 320;
